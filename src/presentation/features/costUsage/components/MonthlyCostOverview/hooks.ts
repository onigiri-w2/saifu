...
import { ExpenseWithCategory, StockWithCategory } from '../../types';
...
export const useStocksWithCategory = (stocks: CostStock[], categories: BudgetingCategory[]) => {
  return useMemo(() => {
    ...
    return stocks
      ...
      .filter((v): v is StockWithCategory => v !== null);
  }, [stocks, categories]);
};

export const useGroupedExpensesByDate = (expenses: Expense[], categories: BudgetingCategory[]) => {
  return useMemo(() => {
    if (expenses.length === 0 || categories.length === 0) return [];

    const result: (LocalDate | ExpenseWithCategory)[] = [];
