...
export class DailyTimeSeries implements ITimeSeries {
  ...
  // otherの中でperiodが被る部分の値を取得して自身から減算する
  subtract(other: DailyTimeSeries): void {
    const subtractPeriod = Period.intersect(this.getPeriod(), other.getPeriod());
    if (subtractPeriod === undefined) return;

    const dates = subtractPeriod.genArray();

    dates.forEach((date) => {
      const index = this.flowpoints.findIndex((p) => p.date.isSame(date));
      const otherPoint = other.flowpoints.find((p) => p.date.isSame(date));
      if (index >= 0 && otherPoint) {
        this.flowpoints[index].value -= otherPoint.value;
      }
    });
  }
